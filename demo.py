...
def run_demo(net, image_provider, height_size, cpu):
    ...
    all_checks = (
        ...
        Checker(message_time_gap, reset_time_gap, kneeDownCheck, "Don't place your knees on the ground"),
            ...
            if count == averaging_duration:
                ...
                if horizontal:
                    ...
                    if begun: 
                        ...
                        if d > 10:
                            tts.BotSpeak("You have now planked for {0} seconds".format(int(time.time()-pose_startt)))
                            last_time_time = time.time()
                        if not correct_pose:                            
                            # if not shk:
                            #     if lower_hip_time > 0:
